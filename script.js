...
$(document).ready(function(){
    ...
    $(document).on( 'click', '.to-cart:not(.read_more)', function(e){
        ...
        if(!th.hasClass('clicked'))
        {
            ...
            fill_prop.item=item;
            fill_prop.basket_props=basket_props;
            fill_prop.set_product_id=setID;
            console.log(setID)
...
/*BX.addCustomEvent('onAsproSkuSetPrice', function(eventdata){
    alert()
    if(typeof window.JCCatalogSection.prototype.OnPriceFromCatalogSection=='function')
        window.JCCatalogSection.prototype.OnPriceFromCatalogSection()
})*/
BX.addCustomEvent('onAjaxSuccess', function(eventdata, eventP1, eventP2){
    if(
        window.JCCatalogSection!=undefined
        && window.JCCatalogSection.OnPriceFromCatalogSection!=undefined
    ){
        window.JCCatalogSection.OnPriceFromCatalogSection()
    }
})

var openPhotoSwipe = function(curId) {
    var selector=document.getElementsByClassName('pswp-ul'),
        elem=selector[0].childNodes,
        items=[],  
        id=0;
    for(var i = 0; i < elem.length; i++) {
        if(elem[i].nodeType==1){            
            id= elem[i].childNodes[1].getAttribute('data-id');
            var imgWidth=elem[i].childNodes[1].getAttribute('data-size-w');
            var imgHeight=elem[i].childNodes[1].getAttribute('data-size-h');
            if(imgWidth>=1500){
                var sootn=imgWidth/imgHeight;
                imgWidth=1500;
                imgHeight=imgWidth/sootn;
            }
            if(imgHeight>=1500){
                var sootn=imgHeight/imgWidth;
                imgHeight=1500;
                imgWidth=imgHeight/sootn;
            }
            items[id]={
                    src: elem[i].childNodes[1].href,
                    w: imgWidth,
                    h: imgHeight,
                    title:  elem[i].childNodes[1].getAttribute('title'),
                    msrc:elem[i].childNodes[1].childNodes[1].src,
                    el: elem[i]
                };
        }
    }
    ...
    var options = {
        ...
        getDoubleTapZoom: function(isMouseClick, item) {      
            if(isMouseClick) {
                ...
                return item.initialZoomLevel < 0.7 ? 1.2 : 1.33; //<---- 4 here
            }
        },
        maxSpreadZoom: 1.2,
    ...
    gallery.listen('imageLoadComplete', function() {
        if(!$(".pswp").hasClass("pswp--zoom-allowed")){
            $(".pswp .pswp__item").on("click",function(){
                $("button.pswp__button--zoom").click();
            })
        }
    });
}
...
var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
        if (mutation.attributeName === "class") {
            ...
            if(attributeValue.indexOf("pswp--open")>-1 && !$("#headerfixed").hasClass("hidden")){
                $("#headerfixed").addClass("hidden");
                $(".banner.SCALE.TOP_HEADER").addClass("hidden");
                $(".header_wrap").addClass("hidden");
                $("header").addClass("hidden");             
            }else if(attributeValue.indexOf("pswp--open")==-1 && $("#headerfixed").hasClass("hidden")){
                $("#headerfixed").removeClass("hidden");
                $(".banner.SCALE.TOP_HEADER").removeClass("hidden");
...
$(document).ready(function () {
    if($(".pswp").length>0){
        ...
        $('.fancybx').on('click',function(e){
            e.preventDefault();
            openPhotoSwipe($(this).data("id")); 
        });
    }
})
